...
export function HourlyForecast() {
  ...
  // Par défaut, sélectionner "aujourd'hui" même si groupedHours[todayLabel] n'existe pas.
  const initialDay = todayLabel
  const [selectedDay, setSelectedDay] = useState(initialDay)

  // Effet pour défiler automatiquement vers l'heure actuelle (lorsque le jour sélectionné est aujourd'hui).
  useEffect(() => {
  ...
  return (
    <Card className="w-full max-w-4xl bg-white/10 border-none shadow-xl rounded-xl transition-transform duration-300 hover:scale-105">
      ...
      <CardContent>
        ...
        <div
          ref={containerRef}
          className="flex space-x-4 overflow-x-auto py-4 snap-x
                     scrollbar-thin scrollbar-thumb-teal-500 scrollbar-track-gray-300
                     hover:scrollbar-thumb-teal-700 transition-colors duration-300"
        >
